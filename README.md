...
The website is built with a microservices architecture but with a single MySQL database (note: services used Sequelize ORM to handle data in a smoother way). There are 5 different projects in my Github that start with the string "PortafolioWeb" that are part of the website:
